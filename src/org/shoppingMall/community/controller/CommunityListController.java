...
public class CommunityListController implements Controller 
{
	...
	public void handle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * int currentPage = 1; CommunityDao dao = CommunityDao.getInstance();
		 * 
		 * String page=request.getParameter("page"); if(page != null) currentPage =
		 * Integer.parseInt(page); //list.jsp 에 page 파라미터를 찾아보세요. int pageSize=5;
		 * //pageSize 를 15 또는 10으로 변경해서 실행해 봅시다. int totalCount = 1;
		 * 
		 * //위의 값들을 이용해서 Paging 객체를 생성하면서 다른 필드값을 계산합니다. Paging paging = new
		 * Paging(currentPage, totalCount, pageSize);
		 ...
		 * request.setAttribute("paging", paging); request.setAttribute("today",
		 * LocalDate.now());
		 */
		int category = 1;
